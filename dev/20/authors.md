# Authors and Citation

## Authors

- **Aksel Thomsen**. Author, maintainer.

- **Novo Nordisk A/S**. Copyright holder.

## Citation

Thomsen A (2026). *S7schema: Generic 'S7' Implementation of Yaml Config
with Schema Based Validation validation*. R package version 0.0.0.9004,
<https://nn-opensource.github.io/S7schema>.

    @Manual{,
      title = {S7schema: Generic 'S7' Implementation of Yaml Config with Schema Based Validation validation},
      author = {Aksel Thomsen},
      year = {2026},
      note = {R package version 0.0.0.9004},
      url = {https://nn-opensource.github.io/S7schema},
    }
